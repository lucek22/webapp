    ...
    if (mode === "a4") {
      ...
      if (onnxSession) {
        try {
          ...
          let maxConf = 0.3; // Minimum detection confidence score threshold
          ...
          if (bestDetection) {
            ...
            const adjustedWidthPx = (bestDetection.x2 - bestDetection.x1) / scale;
            ...
            calibrationSource = "ONNX AI Paper Cutout (A4)";
            console.log(`ONNX calibrated unit scale: ${cmPerUnitSpace.toFixed(2)} cm/unit`);
          }
        } catch (onnxErr) {
          console.warn("Local ONNX pipeline processing failure, running manual fallback:", onnxErr);
        }
      }

      if (!paperWidthNormalized) {
        console.warn("Using hand-landmark estimation fallback for paper width calibration.");
        ...
        calibrationSource = "Hand Track (A4 Fallback)";
      }
    } else if (mode === "card") {
