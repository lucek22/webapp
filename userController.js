...
// =====================================================================
// EXERCISE INSTRUCTIONS & HELP MODAL SYSTEM
// =====================================================================
const exerciseInstructions = {
  posture: {
    title: "Anthropometric Stature Scan",
    steps: [
      "<strong>Enter Subject's Height:</strong> Ensure the correct height is set in centimeters or inches in the calibration sidebar.",
      "<strong>Initiate Biomechanical Tracking:</strong> Click <strong>Start Biomechanical Tracking</strong> to load the camera feed and initialize real-time skeleton modeling.",
      "<strong>Position the Subject:</strong> Subject must stand fully visible in the camera frame, shoes removed, with their back flat against the wall.",
      "<strong>Perform Scan & Capture:</strong> <ul><li><em>Single Snapshot:</em> Align the subject in the desired pose (A-Pose, T-Pose, or Overhead Reach) and click <strong>Start 5s Capture Screenshot</strong>.</li><li><em>Full Autocapture:</em> Start in A-Pose, click <strong>Hands-Free Autocapture</strong>, and transition smoothly through A-Pose, T-Pose, and Overhead Reach on auditory cues.</li></ul>"
    ]
  },
  squat: {
    title: "Overhead Squat Analyzer",
    steps: [
      "<strong>Initiate Biomechanical Tracking:</strong> Click <strong>Start Biomechanical Tracking</strong> to initialize real-time joint tracking and alignment guides.",
      "<strong>Select Testing Plane:</strong> Select the side you want to capture (Frontal or Sagittal view) using the side selector buttons.",
      "<strong>Record Assessment:</strong> Click <strong>Record Squat Assessment</strong> to start recording. Perform 1-3 slow, controlled overhead squats, then click <strong>Record Squat Assessment</strong> again to end and analyze peak valgus/varus, hip, and ankle angles."
    ]
  },
  shoulder: {
    title: "Shoulder Flexion ROM",
    steps: [
      "<strong>Initiate Biomechanical Tracking:</strong> Click <strong>Start Biomechanical Tracking</strong> to initialize active shoulder joint modeling.",
      "<strong>Select Testing Arm:</strong> Choose either the Left or Right side from the control buttons.",
      "<strong>Capture Peak Flexion:</strong> Raise your testing arm straight overhead as far as possible without bending your torso backwards, then capture a snapshot to save the maximum excursion angle."
    ]
  },
  'shoulder-rotation': {
    title: "Shoulder Rotation Assessment",
    steps: [
      "<strong>Initiate Biomechanical Tracking:</strong> Click <strong>Start Biomechanical Tracking</strong> to setup the camera and arm tracker.",
      "<strong>Select Testing Side:</strong> Select the active side (Left or Right) to calibrate joint tracking.",
      "<strong>Set Starting Position:</strong> Lift your elbow to shoulder height, bent at 90°, with your forearm parallel to the ground (0° baseline), then click <strong>Capture Rotation Screenshot</strong> to start recording.",
      "<strong>Perform Range of Motion:</strong> Rotate your forearm slowly upwards/backwards (External Rotation) and then downwards/backwards (Internal Rotation) as far as possible.",
      "<strong>Finalize Assessment:</strong> Click <strong>Capture Rotation Screenshot</strong> again to stop recording and save rotation peaks."
    ]
  },
  'ankle-dorsi': {
    title: "Tibial Inclination (Ankle Dorsiflexion)",
    steps: [
      "<strong>Initiate Biomechanical Tracking:</strong> Click <strong>Start Biomechanical Tracking</strong> to activate ankle joint-angle lines.",
      "<strong>Select Testing Side:</strong> Select either the Left or Right leg to configure the tracking plane.",
      "<strong>Capture Starting Position:</strong> Stand side-on to the camera and click <strong>Capture Rotation Screenshot</strong> to start tracking.",
      "<strong>Perform Weight-Bearing Lunge:</strong> Keeping your back foot stationary, slowly lunge forward, pushing your knee over your toes. <strong>Stop immediately</strong> when the heel of the testing foot begins to lift off the ground.",
      "<strong>Finalize Assessment:</strong> Click <strong>Capture Rotation Screenshot</strong> again to lock and save the peak ankle dorsiflexion/tibial angle."
    ]
  },
  'thoracic-extension': {
    title: "Thoracic Extension Assessment",
    steps: [
      "<strong>Initiate Biomechanical Tracking:</strong> Click <strong>Start Biomechanical Tracking</strong> to align the torso and hip tracking lines.",
      "<strong>Capture Baseline Position:</strong> Stand side-on to the camera, in a neutral upright position, and click <strong>Capture Thoracic Snapshot</strong> to start recording.",
      "<strong>Perform Extension:</strong> Slowly lean backwards from the upper back, arching backwards as far as comfortable (limbo style) before returning to the start position.",
      "<strong>Finalize Assessment:</strong> Click <strong>Capture Thoracic Snapshot</strong> again to stop tracking, analyze peak upper back extension, and sync the snapshot & video to the profile."
    ]
  },
  'hip-rotation': {
    title: "Hip Rotation ROM",
    steps: [
      "<strong>Initiate Biomechanical Tracking:</strong> Click <strong>Start Biomechanical Tracking</strong> to activate seated hip-tracking coordinates.",
      "<strong>Select Testing Hip:</strong> Select either the Left or Right side to position the rotation compass.",
      "<strong>Set Starting Position:</strong> Sit upright with thighs supported, knees bent at 90° hanging off the edge, and click <strong>Capture Rotation Screenshot</strong> to start recording.",
      "<strong>Perform Range of Motion:</strong> Swing your lower leg outward as far as comfortable (Internal Rotation), then inward as far as comfortable (External Rotation).",
      "<strong>Finalize Assessment:</strong> Click <strong>Capture Rotation Screenshot</strong> again to end tracking and export rotation peak metrics."
    ]
  }
};

function initHelpModalSystem() {
  const helpButtons = document.querySelectorAll('.exercise-help-btn');
  const helpModal = document.getElementById('exercise-help-modal');
  ...
  const okBtn = document.getElementById('btn-close-help-modal-ok');

  if (!helpModal || !helpTitle || !helpBody) return;

  function openHelpModal(exerciseKey) {
    const data = exerciseInstructions[exerciseKey];
    if (!data) return;

    helpTitle.textContent = data.title;
    
    // Build beautiful HTML content with numbered steps
    let html = '<ol style="margin: 0; padding-left: 1.25rem; display: flex; flex-direction: column; gap: 0.75rem; list-style-type: decimal;">';
    data.steps.forEach(step => {
      html += `<li style="margin-bottom: 0.25rem; font-family: inherit; line-height: 1.6; color: #cbd5e1;">${step}</li>`;
    });
    html += '</ol>';
  ...
  helpButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      openHelpModal(exerciseKey);
    });
  });

  if (closeBtn) closeBtn.addEventListener('click', closeHelpModal);
  if (okBtn) okBtn.addEventListener('click', closeHelpModal);
...
// Wire up the initialization
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initHelpModalSystem);
} else {
  initHelpModalSystem();
}
